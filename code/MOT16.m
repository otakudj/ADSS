function res=MOT16(name,pattern,visual_det,visual_res)
warning off;
% % param.overlap=0.2;
param.re_id=17;
...
det_ori=load(fullfile('E:\multiple object tracking\Data\MOT16\test',name,'det\det.txt'));
num=1;
frame=length(dir(fullfile('E:\multiple object tracking\Data\MOT16\test',name,'img1')))-2;
det=cell(1,frame);
len=zeros(1,frame);
for i=1:frame
    count=1;
    while det_ori(num,1)==i
        det{i}(count,[1:4,6])=det_ori(num,3:7);
        

        count=count+1;
        num=num+1;
        if num>size(det_ori,1)
            break;
        end
    end
    len(i)=size(det{i},1);
...
if visual_det==1
    k=1;
    for i=1:length(len)
        full_path=fullfile('E:\multiple object tracking\Data\MOT16\test',name,'img1');
...
if visual_res==1
    ...
    for i=1:img_numel
        ...
        while(results(j,1)==i)
            rectangle('Position',results(j,3:6),'Edgecolor','b','LineWidth',3);
            fx = results(j,3)+ 2;
            fy = results(j,4)+ 12;
